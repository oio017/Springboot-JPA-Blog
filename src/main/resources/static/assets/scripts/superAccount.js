...
//			            jqTds[9].innerHTML = '<a class="cancel" href="">Cancel</a>';
							...
							oTable=table.DataTable({
			                   ...
			                   "columnDefs": [{ 'data' : "authList",
								            	...
								            	"render" : function(data, type, row, meta){
													...
													return getMessage('common.approval','승인');
								            	},
								            	...
								            	'data' : function(row, type, val, meta) {
								            		...
								            		if($('#username').val()==row.organ.adminId){
								            			if(row.userId!=$('#username').val()){
								            				content ='<button class="changeMaster btn btn-sm purple btn-outline">마스터 관리자 양도</button>'
								            			}
								            		}else{
								            			content="권한 없음"
								            		}
					...
					var atable = $('#adminTable');
			        
					$.ajax({
						type:'GET',
						url : '/api/super/monitorAccount',
						success:function(returnValue){
							//editedValue = editvalue(returnValue);
							aaTable=atable.DataTable({
					            dom: 'Blfrtp',
					            buttons: [
					                //'copy', 'print', 'pdf', 'excel'
					            ],
			                   ...
			                   "columnDefs": [{ 'data' : "authList",
			                	   				'render' : function(data,type,row, meta){
			                	   					if (row.organ.adminId == row.userId) {
			                	   						return "<i class=\"fa fa-unlock\" aria-hidden=\"true\"></i> 마스터 관리자";
			                	   					}
			                	   					var result="first"
			                	   					if(data.length>0){
			                	   						array = [];
			                	   						...
			                	   						//TODO internationalization
			                	   						result="<i class=\"fa fa-exclamation-triangle\" aria-hidden=\"true\"></i> 권한 없음"
			                	   					}
			                	   					return result;
			                	   				},
			                	   				visible : false,
								            	...
								            	"render" : function(data, type, row, meta){
													...
													if ($('#hasAccountWriteAccess').val() == 'true') {
									            		buttons.push("<a href='#'  class='btn btn-sm blue btn-outline edit editButton' style='padding : 5px; width:50px; text-align: center; margin-top: -5px; margin-bottom: -5px;' data-id='"
																+ data
																+ "'> "+getMessage('common.edit','수정')+"</a>")
													}
													result += buttons.join("&nbsp;&nbsp;")
			        ...
			        table.on('click', '.delete', function (e) {
			            ...
			            if (oTable.row(nRow).data().userId == oTable.row(nRow).data().organ.adminId) {
			        		alert(getMessage("super.cannotDeleteMasterAdmin","마스터 관리자는 삭제할 수 없습니다."));
			        		return;
			        	}
			        ...
			        atable.on('click', '.edit', function (e) {
			        	...
			        	var nRow = $(this).parents('tr')[0];
			            /* Get the row as a parent of the link that was clicked on */
			        	setValues(aaTable.row(nRow).data());
			        	$('.detailModal').modal()
			        });
			        ...
			        table.on('click', '.authEdit', function (e) {
			        	...
			        	if (data.userId == data.organ.adminId) {
			        		if($('#username').val()=='everspin@everspin.co.kr'){
...
function setValues(data){
	...
	$('#input-userId').attr('value',data.userId);
	...
	status="";
	if(data.status==0){
		status+='<select name="status"><option value="0" selected="selected">'+ getMessage('account.allowed','승인')+'</option><option value="1">'+getMessage('account.notAllowed','미승인')+'</option></select>';
	}else{
		status+='<select name="status"><option value="0">'+getMessage('account.allowed','승인') +'</option><option value="1" selected="selected">'+getMessage('account.notAllowed','미승인')+'</option></select>';
	}
...
function writeAuthControl(data){
	...
	result='';
	var authListDisplay=[];
	for(var i=0; i<authCategoryList.length; i++){
...
$('#authSubmit').click(function(){
	...
	$.ajax({
		...
		error:function(e){
			alert(getMessage('common.failed','실패'));
			console.log(e);
		}
...
$('#createNewPasswordButton').click(function(){
	...
	if(isConfirmed){
		$.ajax({
			type: 'POST',
			url : '/api/super/account/password/put/'+id,
			contentType: "application/json; charset=utf-8",
			async : false,
			success:function(returnValue){
	...
	if(tryingbool){
		...
		$.ajax({
			...
			success: function (rsaObject) {
				...
				var rsa = new RSAKey();
				rsa.setPublic(publicKeyModulus,publicKeyExponent);
				var oldNewPassword = $("#formModPut input[name=newPassword]").val();
				var oldPasswordConfirm = $("#formModPut input[name=newPasswordConfirm]").val();
				$("#formModPut input[name=newPassword]").val(rsa.encrypt($("#formModPut input[name=newPassword]").val()));
				$("#formModPut input[name=newPasswordConfirm]").val(rsa.encrypt($("#formModPut input[name=newPasswordConfirm]").val()));
				var params = $('#formModPut').serialize();
				$.ajax({
					type: 'POST',
					url : '/api/super/account/put',
					...
					error:function(e){
						$("#formModPut input[name=newPassword]").val(oldNewPassword);
						$("#formModPut input[name=newPasswordConfirm]").val(oldPasswordConfirm);
						alert('error : '+e.responseJSON.message)
						console.log(e);
						tryingbool = false;
					}
