...
var getQueryData = function(result) {
	...
	searchReq.searchKeyword		=	$('#extension-search-input').val();
...
function getCsv(){
	...
	window.open("/api/log/extension/csv?"+$.param(data), '_blank')
}
...
//								data.organName=organNames
								...
								if(username=="everspin@everspin.co.kr"?true:($('#HasAuthForDetails').val()=='true'?true:false)){
									...
									if(data.tokenDetail==""){
										...
										var detail = JSON.parse(data.tokenDetail)
										...
										console.log(detail)
										var html2 =
										'<table class="table table-full-width table-bordered table-responsive token-detail-table"><tr><th>init_time</th><td>'+(detail["init_time"]==0?detail["init_time"]:timeFormat(detail["init_time"]*1000))+'</td><th>device_arch_type</th><td>'+(detail["device_arch_type"]==undefined?"N/A":detail["device_arch_type"])+'</td>'+'</tr>'
										+'<tr><th>req_mode</th><td>'+detail["req_mode"]+'</td><th>api_mode</th><td>'+detail["api_mode"]+'</td></tr>'
										+'<tr><th>mode_status</th><td>'+detail["mode_status"]+'</td><th>error_message</th><td>'+detail["token_error"]+'</td></tr>'
										+'<tr><th>token_level</th><td>'+detail["token_level"]+'</td><th>token_time</th><td>'+(detail["init_time"]==0?detail["token_time"]:timeFormat(detail["token_time"]*1000))+'</td></tr>'
										+'<tr><th>verification_token_level</th><td>'+detail["token_level"]+'</td><td colspan="2"></td>'+'</tr>'
										+'<tr><th>start_time</th><td>'+(detail["init_time"]==0?detail["start_request_time"]:timeFormat(detail["start_request_time"]*1000))+'</td><th>end_time</th><td>'+(detail["init_time"]==0?detail["end_request_time"]:timeFormat(detail["end_request_time"]*1000))+'</td></tr>'
										+'<tr><th>Trace_API</th><th>Response</th><th colspan="2">time</th></tr>'
										+'<tr><th>'+detail["ptrace_type_1"]+'</th><td>'+detail["ptrace_result_1"]+'</td><td colspan="2">'+(detail["init_time"]==0?detail["ptrace_time_1"]:timeFormat(detail["ptrace_time_1"]*1000))+'</td>'+'</tr>'
										+'<tr><th>'+detail["ptrace_type_2"]+'</th><td>'+detail["ptrace_result_2"]+'</td><td colspan="2">'+(detail["init_time"]==0?detail["ptrace_time_2"]:timeFormat(detail["ptrace_time_2"]*1000))+'</td>'+'</tr>'
										+'<tr><th>'+detail["ptrace_type_3"]+'</th><td>'+detail["ptrace_result_3"]+'</td><td colspan="2">'+(detail["init_time"]==0?detail["ptrace_time_3"]:timeFormat(detail["ptrace_time_3"]*1000))+'</td>'+'</tr>'
										+'<tr><th>'+detail["ptrace_type_4"]+'</th><td>'+detail["ptrace_result_4"]+'</td><td colspan="2">'+(detail["init_time"]==0?detail["ptrace_time_4"]:timeFormat(detail["ptrace_time_4"]*1000))+'</td>'+'</tr>'
										+'</table>'
										$('#specific').html('<table class="table hidden"><tr>'+th+'</tr><tr>'+td+'</tr></table>')
										$('#specific2').html(html2)
	//
	...
	//									)
		...
		clickEventInit: function(){
			...
			$('#extension-search-button').click(function(e){
				if($('#search_column').val()=='status'){
					if(!isInteger($('#extension-search-input').val())){
						alert(getMessage('log.search.onlyNumberWhenTokenStatusCode','토큰 상태 코드 검색시 정수만 입력 가능합니다.'));
						return false;
					}
                    if($('#extension-search-input').val()>2147483647||$('#extension-search-input').val()<-2147483648){
			...
			$('#extension-search-input').keypress(function (e) {
				if(e.keyCode == 13){
					e.preventDefault();
					if($('#search_column').val()=='status'){
						if(!isInteger($('#extension-search-input').val())){
...
$(document).ready(
		function() {
			...
			if($(this).val()=='status'){
				alert(getMessage('log.search.tokenStatusRequiresCode','토큰 상태는 코드를 입력해야 합니다.\n 정상 토큰  : 0 \n 비정상 토큰 : 1 \n 접속장애 : 2\n 시간초과 토큰 : 3\n Canceled : 4\n 기본 토큰 : 5\n'));
			}
			if($(this).val()=='blocked'){
		...
		"columnDefs" : [		
			...
			},
			{
				'render' : function(data, type, row, meta) {
					var result;
					if(data == 0){
						result = getMessage('token.normalToken0','정상 토큰(0)');
					}else if(data == 1){
						result = getMessage('token.abnormalToken1','비정상 토큰(1)');
					}else if(data == 2){
						result = getMessage('token.connectionFailure2','접속 장애(2)');
					}else if(data == 3){
                        result = getMessage('token.connectionFailureToken3', '시간초과 토큰(3)');
                    }else if(data == 4){
                        result = getMessage('token.canceledToken4', '취소 토큰(4)');
                    }else if(data == 5){
						result = getMessage('token.basicToken5', '기본 토큰(5)');
					}else if(data == 16){
						result = getMessage('token.bypassToken16', '바이패스 토큰(16)');
					}else{
						result = "N/A"
					}
			...
			},
			{
				...
				'visible' : username=="everspin@everspin.co.kr"?true:($('#HasAuthForDetails').val()=='true'?true:false),
				targets : [11]
			},{
                "searchable" : false,
