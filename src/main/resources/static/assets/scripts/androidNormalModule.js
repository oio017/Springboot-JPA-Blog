...
var inquireToServer = function () {
    $.ajax({
        url: '/api/super/module/normal/version/android',
        type: 'GET',
        async:false,
...
var initAppVersionMap = function () {
    $.ajax({
        url: '/api/super/module/normal/appVersionMap/android',
        type: 'GET',
        async:false,
        success: function (result) {
            console.log(result)
...
function makeTable(returnValue) {
    ...
    table.on("click",".normal-detail-button", function(e){
        ...
        location.href="/super/module/android/normal/"+data.version;
    })
    ...
    table.on("click",".normal-randomGetModule-button", function(e){
        ...
        while(!isGoodCount){
            ...
            if(isNaN(tempInputValue)) {
                ...
                isGoodCount=true
                window.location='/api/download/module/normal/android/'+data.version+'/random/'+count
            }
        }
    });
 ...
 // if (confirm(getMessage('common.deleteConfirmMessage','정리시 복구할 수 없습니다. 그래도
// 진행하시겠습니까?')) == true) { if
// (confirm(getMessage('common.moduleDeleteConfirmMessage', '파일이 삭제될 경우 해당 버전의
 // 모듈은 새로 등록될 수 없습니다. 그래도 삭제하시겠습니까?')) == false) { return false; }else{ }
	// }else{
 // return false; }
 
        /*
		 * //DataTable 라이브러리를 가져다쓰는데 DataTable에서 제공하는 함수들은 row함수는 매개변수로 Table의
		 * tr dom을 변수로 받으면 dataTable이 그 tr에 해당하는 정보를 반환 그 tr에 해당하는 데이터를 반환한다.
		 * DataTable 에 Data는 Array를 반환 하기는 하는데 순수한 어래이는 아니고 dataTable에서
		 * Javascript의 Array를 상속받은 커스텀한 Array를 내보낸다.
		 */
        var nRow = $(this).parents('tr')[0];
        var data = oTable.row(nRow).data();
    ...
    table.on("click",".normal-delete-button", function(e) {
        ...
        $.ajax({
            type:'GET',
            url:'/api/super/module/normal/latestVersion/android',
            contentType: "application/json; charset=utf-8",
            async: false,
            success: function (returnValue) {
                if(data.version==returnValue){
